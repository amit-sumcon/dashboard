                        ...
                        <div className='flex flex-col gap-3 w-full'>
                            <span className={`${isCollapsed && 'hidden'} text-sm text-gray-400`}>Pages</span>
                            {
                                pages.map((item, index) => (
                                    <LinkItem isActive={isActive(item.path)} path={item.path} label={item.label} icon_name="list-ol" key={index} isCollapsed={isCollapsed} />
                                ))
                            }
                        </div>
                        <div className='flex flex-col gap-3 w-full'>
                            <span className={`${isCollapsed && 'hidden'} text-sm text-gray-400`}>Settings</span>
                            <LinkItem isActive={isActive('/settings/navbar-setting')} label="Navbar Settings" path="/settings/navbar-setting" icon_name="list-ol" isCollapsed={isCollapsed} />
                            <LinkItem isActive={isActive('/settings/footer-setting')} label="Footer Settings" path="/settings/footer-setting" icon_name="list-ol" isCollapsed={isCollapsed} />
                        </div>
                        <div className='flex flex-col gap-3 w-full'>
                            <span className={`${isCollapsed && 'hidden'} text-sm text-gray-400`}>User</span>
                            <LinkItem isActive={isActive('/user/list')} label="List" path="/user/list" icon_name="list-ol" isCollapsed={isCollapsed} />
                            <LinkItem isActive={isActive('/user/view')} label="View" path="/user/view" icon_name="street-view" isCollapsed={isCollapsed} />
                        </div>
                        <div className='flex flex-col gap-3 w-full'>
                            <span className={`${isCollapsed && 'hidden'} text-sm text-gray-400`}>Roles & Permissions</span>
                            <LinkItem isActive={isActive('/roles-permissions/roles')} label="Roles" path="/roles-permissions/roles" icon_name="check-shield" isCollapsed={isCollapsed} />
                            <LinkItem isActive={isActive('/roles-permissions/permissions')} label="Permissions" path="/roles-permissions/permissions" icon_name="lock-alt" isCollapsed={isCollapsed} />
                        </div>
                    </div>
                </div>
            </div>
